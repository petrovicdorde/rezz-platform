...
import type { CreateVenueRequest, WorkingHours } from '@/lib/types/venue.types';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (selected) {
                        field.onChange(
                          field.value.filter((v: PaymentMethod) => v !== pm.value),
                        );
                      } else {
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        >
          {createVenue.isPending ? t('common.loading') : t('venue.add_new')}
        </Button>
      </div>
    </form>
  );
