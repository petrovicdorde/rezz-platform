...
function detectPlatformFromUrl(url: string): SocialPlatform {
  try {
...
function renderIcon(platform: SocialPlatform): React.ReactNode {
  switch (platform) {
    case 'INSTAGRAM':
      return (
        <svg
...
export function SocialLinkIcon({
  url,
  platform,
}: SocialLinkIconProps): React.JSX.Element {
  ...
  const resolved: SocialPlatform = known.includes(platform as SocialPlatform)
