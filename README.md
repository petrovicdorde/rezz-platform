...
The review queue lives inside the existing Users page, under the "Crna lista" (blacklist) filter. Selecting that filter reveals a sub-filter row with two pills — "Lista" (the list of currently blacklisted users) and "Žalbe" (the appeals queue). Switching to "Žalbe" replaces the user list with three status chips — Pending, Approved, Rejected — defaulting to Pending. Each card shows the guest's name and email, the original block reason, the guest's message, the submission timestamp, and (for decided appeals) the admin's note. While viewing pending appeals, each card has Approve and Reject buttons. Both buttons open a confirmation dialog with an optional admin-note textarea. Confirming runs the decision in a database transaction.
